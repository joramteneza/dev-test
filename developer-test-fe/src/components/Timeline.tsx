...
export function Timeline({ goals }: TimelineProps) {
  ...
  const years = Array.from(
    new Set(goals.map((goal) => dayjs(goal.date).year().toString()))
  ...
  const calculateSpacing = (currentGoal: Goal, index: number) => {
    if (index === 0 || !showTimeBetweenEvents || !currentGoal.timestamp)
      return 0;
  ...
  return (
    <div className="relative">
        ...
        <div className="flex items-center space-x-2 bg-white rounded-md px-3 py-1 border border-light-blue-200 cursor-pointer">
          <Checkbox
            ...
            onCheckedChange={(checked) =>
              setShowTimeBetweenEvents(checked === true)
            }
            className="cursor-pointer"
          />
          <label
            htmlFor="showTimeBetween"
            className="text-xs font-medium text-light-blue-200 cursor-pointer"
          >
      ...
      {showIndicator && visibleDate && (
        <div className="fixed top-20 right-4 z-50 px-3 py-1 bg-white shadow-md border rounded-md text-xs text-gray-600">
          {visibleDate}
        </div>
      )}
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isDesktop
                    ? "mx-auto grid grid-cols-2 gap-4 items-center"
                    : "pl-12",
                  index > 0 && spacing > 0
                    ? `mt-${Math.min(24, Math.floor(spacing / 4))}`
                    : "mt-5"
                )}
                style={
                  index > 0 && spacing > 0 ? { marginTop: `${spacing}px` } : {}
                }
                data-date={goal.date}
              >
                {/* Timeline dot */}
                ...
                >
                  <Card>
                    ...
                    ></div>
                    <CardHeader>
                      <CardTitle className="text-dark-gray text-xs">
                        {goal.name}
                      </CardTitle>
                      <p className="text-light-gray text-xs">{date}</p>
                    </CardHeader>
                    <CardContent className="px-10">
                      <p className="text-gray-700 text-tiny italic">
                        {goal.description}
                      </p>
                    </CardContent>
                  </Card>
                ...
                {isDesktop && (
                  <div className={isLeft ? "col-start-2" : "col-start-1"}></div>
                )}
              </div>
            );
          })}
